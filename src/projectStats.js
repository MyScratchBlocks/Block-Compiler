...
const { addMessage } = require('./messages'); // ✅ import addMessage
...
router.post('/api/projects/:id/:action/:username', (req, res, next) => {
  ...
  try {
    const updatedData = updateStats(filePath, statKey);

    // ✅ Notify author if available
    if (updatedData.author?.username && updatedData.title) {
      const message = `${username} ${action === 'love' ? 'liked' : 'favorited'} your project <a href="/projects/#${id}">${updatedData.title}</a>`;
      addMessage(updatedData.author.username, message);
    }

    res.json({ message: `${statKey} incremented`, stats: updatedData.stats });
  } catch (err) {
...
router.post('/api/:id/views/:username', (req, res) => {
  ...
  try {
    const updatedStats = updateStats(filePath, 'views').stats;
